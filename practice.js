...
function divider(numbersArray){
  ...
  for(i=0; i<numbersArray.length; i++){
    if(numbersArray[i] % 2 ==0){arrEven.push(numbersArray[i])}
    else {arrOdd.push(numbersArray[i])}
  }
  let arr = [arrEven, arrOdd];
  return arr;
};
...
function removeItem(myGroceryList, item){
  ...
  for (i=0; i < myGroceryList.length; i++){
    if (myGroceryList[i] != item){newList.push(myGroceryList[i])}
  }
  return newList;
};
